...
export default function AgencyDetail() {
  ...
  // Hub được đại lý xin trong yêu cầu vừa duyệt — để pre-check sẵn trong HubGrantList,
  // không áp dụng khi mở "+ Cấp thêm Hub" thủ công (không gắn với yêu cầu cụ thể nào)
  const [defaultHubSelection, setDefaultHubSelection] = useState<string[] | undefined>(undefined)
  ...
  const pendingRequestCount =
    ...
    carrierRequests.filter(r => r.agencyId === agency.id && r.status === 'pending').length
      ...
      <div style={{ maxWidth: 1024, margin: '0 auto', padding: '24px 80px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          <ArrowLeftOutlined
            ...
            onClick={() => navigate('/super-admin/agencies')}
          />
          <span style={{ fontSize: 24, fontWeight: 600, color: C_TEXT_PRIMARY, flex: 1 }}>Xem chi tiết</span>
          <button
          ...
          >
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: pendingRequestCount > 0 ? '#3B82F6' : C_TEXT_SECONDARY }}>
              Xem yêu cầu
            </span>
          </button>
        </div>
                    ...
                    {ALL_CARRIERS.map((carrier) => {
                      ...
                      return (
                        <div key={carrier.key} style={{ display: 'flex', flexDirection: 'column', gap: 0, border: `1px solid ${enabled ? carrier.color + '40' : (pendingReq ? '#FCD34D' : C_BORDER)}`, borderRadius: 8, overflow: 'hidden', background: enabled ? carrier.color + '08' : (pendingReq ? '#FFFBEB' : '#FAFAFA') }}>
                          ...
                          {pendingReq?.requestedHubIds && pendingReq.requestedHubIds.length > 0 && (
                            <div style={{ padding: '8px 12px', borderTop: '1px solid #FDE68A', background: '#FFFDF0', fontSize: 12, color: '#92400E' }}>
                              <span style={{ fontWeight: 600 }}>Địa điểm được yêu cầu: </span>
                            ...
                            <div style={{ padding: '10px 12px', borderTop: `1px solid ${carrier.color}20` }}>
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                                <span style={{ fontSize: 12, fontWeight: 600, color: C_TEXT_LABEL }}>
                                  Địa điểm gửi hàng đã cấp ({(agency.clientHubIds ?? []).length})
                                </span>
                                {addingHubFor !== carrier.key && (
                                  <button
                                    onClick={() => { setAddingHubFor(carrier.key); setDefaultHubSelection(undefined) }}
                              ...
                              {(agency.clientHubIds ?? []).length === 0 ? (
                                <div style={{ fontSize: 12, color: C_TEXT_SECONDARY }}>Chưa cấp địa điểm gửi hàng nào.</div>
                              ) : (
                              ...
                              {addingHubFor === carrier.key && (
                                <div style={{ marginTop: 8 }}>
                                  <HubGrantList
                                    ...
                                    defaultSelected={defaultHubSelection}
                                    onGrant={(hubIds) => {
                                      hubIds.forEach(hubId => grantAdditionalHub(agency.id, hubId))
                                      setAddingHubFor(null)
                                      forceRender(n => n + 1)
                                    }}
                                    onClose={() => setAddingHubFor(null)}
                                  />
                                </div>
                              )}
                            </div>
