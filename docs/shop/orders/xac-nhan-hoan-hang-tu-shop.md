...
- **Khoảng hở — câu xác nhận sai với đơn Thư:** AC5 nói đơn Thư "không bị ảnh hưởng" vì `isLetterReturnCase` chặn nút SAU khi đã `returning`. Đúng — nhưng AC5 chỉ bảo vệ giai đoạn sau xác nhận. Ở giai đoạn `redelivery` (chưa phải `returning`), `isLetterReturnCase` chưa đúng → đơn Thư vẫn qua `isReturnEligible` và thấy nút "Hoàn hàng". Khi bấm, câu xác nhận hiện tại (dòng 2706 `Orders.tsx`) dùng chung nội dung với Hàng hoá — "nhà vận chuyển sẽ chuyển hoàn về địa chỉ trả hàng đã đăng ký" — sai cả về cơ chế (đơn Thư về qua đại lý trước, xem SHOP-ORDER-8) lẫn về field tham chiếu (chưa implement thật). Xem **[SHOP-ORDER-35](xac-nhan-hoan-hang-cau-xac-nhan-sai-voi-don-thu.md)** để biết chi tiết gap và đề xuất fix.
