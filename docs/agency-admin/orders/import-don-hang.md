...
2. Modal "Import đơn hàng" mở ra, hiện rõ đúng thứ tự 10 cột cần có trong file
3. Bấm vùng upload, chọn file `.xlsx`/`.xls`
...
2. Parse thật bằng `XLSX.read()` trên nội dung file upload (không phải progress bar giả) — đọc toàn bộ sheet đầu tiên qua `sheet_to_json({ header: 1 })`, bỏ dòng tiêu đề và dòng trống
3. Validate từng dòng: `Mã shop` phải khớp 1 shop thuộc đại lý hiện tại (`agencyShops`); `Loại đơn` chỉ nhận "Hàng hoá" hoặc "Thư"; các trường bắt buộc (Khách hàng, SĐT, Địa chỉ) không rỗng; Khối lượng phải là số dương; COD nếu có phải là số; Phí ship phải là số không âm
4. Dòng hợp lệ → build `Order` object, set `sendKind` theo Loại đơn:
...
5. `senderName`/`senderPhone` lấy từ `ownerName`/`phone` của shop tương ứng trong `shops.json`
6. Mỗi dòng hợp lệ gọi `addOrder()` (dùng chung cơ chế với `orderStore.ts`) → `refreshOrders()` load lại danh sách từ store
